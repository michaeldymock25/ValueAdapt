...
defn <- matrix(c(
  ...
  "$\\text{E}_{\\boldsymbol{\\Theta}|\\boldsymbol{x}_{1:\\bar{n}_j}} \\left[\\sum_{d=1}^D p_d \\times NB(d,\\boldsymbol{\\Theta},t_j,t_{j+1})\\right] + \\text{E}_{\\boldsymbol{X}_{(\\bar{n}_j+1):\\bar{n}_{j+1}}} \\left[\\underset{d}{\\text{max }} \\text{E}_{\\boldsymbol{\\Theta}|\\boldsymbol{x}_{1:\\bar{n}_j},\\boldsymbol{X}_{(\\bar{n}_j+1):\\bar{n}_{j+1}}} [NB(d,\\boldsymbol{\\Theta},t_{j+1},t_H)]\\right] - \\underset{d}{\\text{max }} \\text{E}_{\\boldsymbol{\\Theta}|\\boldsymbol{x}_{1:\\bar{n}_j}} \\left[NB(d,\\boldsymbol{\\Theta},t_j,t_H)\\right] - (\\delta + \\gamma \\times \\bar{n}_j)$"
...
$$
\sum_{i_k=1}^{\bar{n}_{jk}} X_{i_k} \sim \text{Binomial}(\bar{n}_{jk}, p_k) \quad \forall k \in \{\text{II},\text{MV}\}
$$
